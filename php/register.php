<?php
	...
	if(!isset($_POST['number']) || strlen($_POST['number']) > 13 || mb_substr($_POST['number'], 0, 3) == '+44' || !preg_match('/^[0-9 ]+$/', mb_substr($_POST['number'], 0, 3))) {
		$errors[] = 4;
	}
		...
		if(isset($_POST['csrf_token']) && validateToken($_POST['csrf_token'])) {
			...
			if($C) {
				...
				if($res && $res->num_rows === 0) {
					...
					$id = sqlInsert($C, 'INSERT INTO users VALUES (NULL, ?, ?, ?, ?, 0)', 'ssss', $_POST['name'], $_POST['email'], 'user', $hash);
					if($id !== -1) {
